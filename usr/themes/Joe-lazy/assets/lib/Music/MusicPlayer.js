...
class MusicPlayer {
  ...
  initStorageMusic() {
    ...
    if (
      ...
      music["artist"] != storage_music["artist"] &&
      music["name"] != storage_music["name"]
    ) {
      return;
    }
  ...
  setMusic() {
    ...
    for (let key in music) {
      // 音频名称
      if (!music[key]["name"]) {
        music[key]["name"] = music[key]["title"]
          ? music[key]["title"]
          : "歌曲".key;
      }
      // 音频作者
      if (!music[key]["artist"]) {
        music[key]["artist"] = music[key]["author"]
          ? music[key]["author"]
          : "无信息";
      }
      // 音频歌词

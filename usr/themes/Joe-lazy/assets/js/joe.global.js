document.addEventListener("DOMContentLoaded", () => {
  ...
  /* 初始化昼夜模式 */
  {
    ...
    const toggleDarkMode = (isDark) => {
      $("html").attr("data-night", isDark ? "night" : null);
      $(".joe_action_item.mode .icon-1").toggleClass("active", isDark);
      $(".joe_action_item.mode .icon-2").toggleClass("active", !isDark);
    };
    ...
    $(".joe_action_item.mode").click(() =>
      toggleDarkMode(!$("html").attr("data-night"))
    );
  }
  ...
  /* 侧边栏舔狗日记 */
  {
    if ($(".joe_aside__item.flatterer").length) {
      ...
      const toggle = () => {
        $(".joe_aside__item.flatterer .content").html(
          arr[random(0, arr.length - 1)].replace(
            /\*\*(.*?)\*\*/g,
            "<mark>$1</mark>"
          )
        );
        $(".joe_aside__item.flatterer .content").attr(
          "class",
  ...
  /* 激活评论提交 */
  {
    ...
    var appendComment = function (comment) {
      // 评论列表
      var el = $("#comment_module > .comment-list");
      if (0 != comment.parent) {
        // 子评论则重新定位评论列表
        var el = $("#comment-" + comment.parent);
        // 父评论不存在子评论时
        if (el.find(".comment-list__item-children").length < 1) {
          $(
            '<div class="comment-list__item-children"><ol class="comment-list"></ol></div>'
          ).appendTo(el);
        } else if (
          el.find(".comment-list__item-children > .comment-list").length < 1
        ) {
          $('<ol class="comment-list"></ol>').prependTo(
            el.find(".comment-list__item-children")
          );
        }
        el = $("#comment-" + comment.parent)
          .find(".comment-list__item-children")
          .find(".comment-list");
      }
      if (0 == el.length) {
        $('<ol class="comment-list"></ol>').appendTo($("#comment_module"));
        el = $("#comment_module > .comment-list");
      }
      // 评论html模板，根据具体主题定制
      const {
        coid,
    ...
    <li class="comment-list__item">
        <div class="comment-list__item-contain" id="comment-${coid}">
            <div class="term">
                <div class="zetx">
                    <img   width="48" height="48" class="avatar lazyload a" src="${avatarlazyload}" data-src="${avatarbymail}" alt="头像"> 
                    <img   width="48" height="48" class="avatar lazyload b" src="${avatarlazyload}" data-src="${letter_avatar}" alt="头像">
                </div>
                <div class="content">
                    <div class="user">
                        <span class="author">${author}
                            <div class="agents">${ips}</div>
                        </span>
                        ${owner} ${status}
                        <div class="agent">${agent}</div>
                    </div>
                    <div class="substance">${parentcontent} ${content}</div>
                    <div class="handle">
                        <time class="date" datetime="${datetime}">${datetime}</time> 
                        <span class="reply joe_comment__reply" data-id="comment-${coid}" data-coid="${coid}">
      ...
      $(html).prependTo(el);
      window.scrollTo({
        top: $(el).offset().top - $(".joe_header").height() - 15, // 你可能需要根据你的页面布局调整这个值
        behavior: "smooth",
      });
      activateComment();
      var $title = $(".joe_comment__title");
      var num = parseInt($title.text().match(/(\d+)/)[0]) + 1;
      $title.text($title.text().replace(/\d+/, num));
    };

    if ($(".joe_comment").length) {
      $(".joe_comment__respond-form").on("submit", function (e) {
        e.preventDefault();
        const action = "?action=comment";
        ...
        const url = $(
          ".joe_comment__respond-form .head input[name='url']"
        ).val();
        let text = $(
        ...
        const loadingMsg = Qmsg.loading("正在发送评论···");
  ...
  /* 设置评论回复网址为新窗口打开 */
  {
    $(".comment-list__item .term .content .user .author a").each(
      (index, item) => $(item).attr("target", "_blank")
    );
  }
      ...
      $.ajax({
        ...
        success(res) {
                    ...
                    (_) =>
                      `<li class="item" data-text="${_.data}">${
                        key === "颜文字"
                          ? `${_.icon}`
                          : `<img   src="${
                              window.Joe.THEME_URL + _.icon
                            }" title="${/.*?\((.*?)\)/.exec(_.data)[1]}"	alt="${
                              /.*?\((.*?)\)/.exec(_.data)[1]
                            }"/>`
                      }</li>`
                  )
          ...
          $(".joe_owo__contain").html(`
						<div class="seat"><i class="fa fa-fw fa-smile-o"></i><span class="title">表情</span></div>
						<div class="box">
							${scrollStr}
...

  {
    $(".privateComment").click(function () {
      if ($("#private").prop("checked")) {
        ...
        $(this).css("background", "");
        $(this).css("color", "");
        $("#comment").attr("placeholder", "来都来啦，说点什么吧！");
      } else {
        $(".fa-unlock-alt").hide();
        $(".fa-lock").show();
        $("#private").prop("checked", true);
        $(this).css("background", "var(--theme)");
        $(this).css("color", "rgb(255, 255, 255)");
        $("#comment").attr(
          "placeholder",
          "正在隐私评论中,仅发布者和作者可见，说点什么吧.."
        );
      }
    });
  }

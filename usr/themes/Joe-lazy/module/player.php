...
<head>
	...
	<style>
		*{margin:0;padding:0;-webkit-tap-highlight-color:transparent;outline:none;text-decoration:none}html,body,#dplayer{width:100%;height:100%}
	</style>
</head>

<body>
	<div id="dplayer"></div>
	<script src="<?= joe\theme_url('assets/lib/hls/hls.min.js'); ?>"></script>
	<script src="<?= joe\theme_url('assets/lib/DPlayer/DPlayer.min.js'); ?>"></script>
	<script>
		window.videoPlayer = new DPlayer({container:document.getElementById('dplayer'),// 播放器容器元素
			...
			video:{url:'<?= $_GET['url'] ?>',pic:<?= isset($_GET['pic']) ? "'" . $_GET['pic'] . "'":'null'  ?>,}})
	</script>
</body>

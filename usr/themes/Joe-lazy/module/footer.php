...
?>
	<meting-js fixed="true" preload="metadata" mutex="true" mini="true" volume="0.3" autotheme="true" storage="AllFixed" order="<?= $this->options->JMusicOrder ?>" server="<?= $this->options->JMusicServer ?>" type="<?= $this->options->JMusicType ?>" id="<?= $this->options->JMusicId ?>" <?= $this->options->JMusicPlay == 'on' ? 'autoplay="true"' : null ?>></meting-js>
	<script src="<?= joe\theme_url('assets/lib/Music/Meting.min.js'); ?>"></script>
<?php
}
...
<script>
	...
	?>
	console.log('\n' + ' %c Theme By Joe懒人版' + ' %c https://letanml.xyz/web-build/21.html ' + '\n', 'color: #fadfa3; background: #030307; padding:5px 0;', 'background: #fadfa3; padding:5px 0;');
	<?php $this->options->JCustomScript() ?>
</script>

<?php $this->options->JCustomBodyEnd() ?>
<?php if ($this->options->JDynamic_Background !== "off" && !preg_match('/windows phone|iphone|android/i', $_SERVER['HTTP_USER_AGENT'])) : ?>
	<script src="<?= joe\theme_url('assets/lib/backdrop/' . $this->options->JDynamic_Background) ?>"></script>
<?php endif; ?>

<script>
	window.onload = () => setTimeout(() => document.querySelector(".lazyparent").style.display = "none", 1000);
</script>

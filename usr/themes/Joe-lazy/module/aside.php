<aside class="joe_aside">
				...
				<?php if ($item->have()) : ?>
					<?php while ($item->next()) : ?>
						<li class="item">
							<div class="user">
								<div class="zetx">
									<img   width="40" height="40" class="avatar lazyload a" src="<?php joe\getAvatarLazyload() ?>" data-src="<?php joe\getAvatarByMail($item->mail) ?>" alt="<?php $item->author(false) ?>" />
									<img   width="40" height="40" class="avatar lazyload b" src="<?php joe\getAvatarLazyload() ?>" data-src="<?php joe\letter_avatar($item->author); ?>" alt="<?php $item->author(false) ?>" />
								</div>
								<div class="info">
							...
							<div class="reply">
								<a class="link" href="<?php _parseAsideLink($item->permalink); ?>">
									<?php echo _parseCommentReply($item, 'aside'); ?>
								</a>
							</div>
						</li>
					<?php endwhile; ?>
	...
	<?php if ($this->options->JAside_Weather_Key) : ?>
		...
		<script>
			document.addEventListener("DOMContentLoaded",()=>{if($(".joe_aside__item.weather").length){const key=$(".joe_aside__item.weather").attr("data-key");const style=$(".joe_aside__item.weather").attr("data-style");const aqiColor={1:"FFFFFF",2:"4A4A4A",3:"FFFFFF",};window.WIDGET={CONFIG:{layout:2,width:"220",height:"270",background:style,dataColor:aqiColor[style],language:"zh",key,},}}});
		</script>

			...
			<div id="<?php $this->respondId(); ?>" class="joe_comment__respond">
				...
				<form method="post" class="joe_comment__respond-form" action="<?php $this->commentUrl() ?>" data-type="text">
					...
					<div class="body">
						...
						if ($this->options->Jcomment_draw == 'on') {
						?>
							<textarea class="text joe_owo__target" id="comment" name="text" value="" autocomplete="new-password" placeholder="来都来啦，说点什么吧..."></textarea>
					...
					<div class="foot">
						...
						<div class="Comment_privacy">
							<div class="comment_box privateComment">
								<i class="fa fa-fw fa-unlock-alt"></i>
								<i class="fa fa-fw fa-lock" style="display: none;"></i>
								<span class="title">密语</span>
								<input type="checkbox" style="display: none;" id="private" />
							</div>
						</div>
						<div class="submit">
...
{ ?>
	<li class="comment-list__item">
		<div class="comment-list__item-contain" id="<?php $comments->theId(); ?>">
			<div class="term">
				<div class="zetx">
					<img   width="48" height="48" class="avatar lazyload a" src="<?php joe\getAvatarLazyload() ?>" data-src="<?php joe\getAvatarByMail($comments->mail); ?>" alt="头像" />
					<img   width="48" height="48" class="avatar lazyload b" src="<?php joe\getAvatarLazyload() ?>" data-src="<?php joe\letter_avatar($comments->author); ?>" alt="头像" />
				</div>
				<div class="content">
					<div class="user">
						<span class="author">
							...
							<?php if ($comments->authorId === $comments->ownerId) : ?>
								<svg t="1664161860238" class="icon" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="5642" width="18" height="18">
									<path d="M918.4768 439.9616l-72.4992-72.4992V264.96c0-49.7152-40.448-90.112-90.1632-90.112H653.312l-72.4992-72.4992c-17.0496-17.0496-39.68-26.4192-63.744-26.4192s-46.6944 9.3696-63.744 26.4192L380.8768 174.7968H278.3232c-49.7152 0-90.1632 40.448-90.1632 90.112v102.5024L115.712 439.9616a89.45664 89.45664 0 0 0-26.4192 63.744c0 24.064 9.3696 46.6944 26.4192 63.744l72.4992 72.4992v102.5024c0 49.7152 40.448 90.1632 90.112 90.1632h102.5536l72.4992 72.4992c17.5616 17.5616 40.6528 26.368 63.744 26.368 23.0912 0 46.1824-8.8064 63.744-26.368l72.4992-72.4992h102.5024c49.7152 0 90.1632-40.448 90.1632-90.1632v-102.5024l72.4992-72.4992c17.0496-17.0496 26.4192-39.68 26.4192-63.744-0.0512-24.064-9.472-46.7456-26.4704-63.744z" fill="#7B79FF" p-id="5643"></path>
									<path d="M511.61770667 658.90986667c-7.53664 0-14.90944-2.62144-20.80768-7.48202667l-139.264-114.52416a32.768 32.768 0 0 1-4.47829334-46.14826667 32.768 32.768 0 0 1 46.14826667-4.47829333l111.90272 92.02346667 130.03434667-185.9584a32.768 32.768 0 0 1 45.65674666-8.08277334 32.768 32.768 0 0 1 8.08277334 45.65674667l-150.35050667 215.01269333a32.93184 32.93184 0 0 1-22.39146667 13.70794667c-1.52917333 0.16384-3.05834667 0.27306667-4.53290666 0.27306667z" fill="#FFFFFF" p-id="5644"></path>
								</svg>
							<?php endif; ?>
							<div class="agents">
						...
						<?php endif; ?>
						<div class="agent"><?php XQUserAgent_Plugin::render($comments->agent); ?></div>
					</div>
					<div class="substance">
						<?php echo joe\getParentReply($comments->parent) ?>
						<?php echo _parseCommentReply($comments); ?>
					</div>
					<div class="handle">

...
/* 过滤评论回复 */
function _parseCommentReply(object $comment, string $context = 'comment'): string
{
	if (_checkXSS($comment->content)) {
		return "该回复疑似异常，已被系统拦截！";
	}

	$text = strip_tags($comment->content);
	if (strpos($text, '[secret]') !== false && strpos($text, '[/secret]') !== false) {
		$rememberedEmail = Typecho_Cookie::get('__typecho_remember_mail');
		$db = Typecho_Db::get();
		$user = Typecho_Widget::widget('Widget_User');

		$parentCommentId = $comment->parent;
		$visibleEmails = [$user->mail,  $comment->mail];
		while ($parentCommentId != 0) {
			$parentComment = $db->fetchRow(
				$db->select('parent', 'mail')
					->from('table.comments')
					->where('coid = ?', $parentCommentId)
					->limit(1)
			);
			$parentCommentId = $parentComment['parent'];
			$visibleEmails[] = $parentComment['mail'] ?? '';
		}

		if (
			(!empty($rememberedEmail) && in_array($rememberedEmail, $visibleEmails)) ||
			$user->group == 'administrator'
		) {
			$text = str_replace(['[secret]', '[/secret]'], '', $text);
		} else {
			return '此条为密语，仅发布者及管理员可见！';
		}
	}
	...
	$replacements = [
		'comment' => '<img     class="lazyload draw_image" src="data:image/gif;base64,R0lGODlhAQABAAAAACH5BAEKAAEALAAAAAABAAEAAAICTAEAOw==" data-src="$1" alt="画图"/>',
		'leaving' => '<img     class="draw_image" src="$1" alt="画图"/>',
		'aside'   => '# 图片回复'
	];

	return preg_replace('/\{!\{([^\"]*)\}!\}/', $replacements[$context] ?? '$1', $text);
}

		...
		foreach ($sticky_arr as $cid) {
			...
			if ($item->next()) {
				$result[] = array(
					...
					"created" =>  joe\formatTime($item->created),
					"title" => $item->title,
...
{
	$self->response->setStatus(200);
	$cid = filter_var($self->request->cid, FILTER_VALIDATE_INT, ["options" => ["min_range" => 1]]);

	if ($cid === false) {
		return $self->response->throwJson(["code" => 0, "data" => "非法请求！已屏蔽！"]);
	}
	...
	if ($row) {
		...
		$self->response->throwJson(["code" => 0, "data" => null]);
	}
}
...
function _handleAgree($self)
{
	...
	if ($cid === false || !in_array($type, ['agree', 'disagree'])) {
		return $self->response->throwJson(["code" => 0, "data" => "非法请求！已屏蔽！"]);
	}
	...
	if ($row) {
		$agree = $type === "agree" ? (int)$row['agree'] + 1 : (int)$row['agree'] - 1;
		$db->query($db->update('table.contents')->rows(['agree' => $agree])->where('cid = ?', $cid));
		$agree = $db->fetchRow($db->select('agree')->from('table.contents')->where('cid = ?', $cid))['agree'];
		$self->response->throwJson([
			"code" => 1,
			"data" => ['agree' => number_format($agree)]
		]);
	} else {
		$self->response->throwJson(["code" => 0, "data" => null]);
	}
